...
/// <summary>
/// Applies the embedded PostgreSQL migration script set via DbUp. Scans only this assembly, so its scripts
/// can never run against another engine.
/// </summary>
public sealed class PostgresDbUpRunner : IDbUpRunner
{
    private readonly string _connectionString;
    private readonly ILogger<DatabaseStartupService> _logger;

    public PostgresDbUpRunner(string connectionString, ILogger<DatabaseStartupService> logger)
    {
    ...
    {
        var upgrader = PostgresqlExtensions.PostgresqlDatabase(
                DeployChanges.To,
                _connectionString)
            .WithScriptsEmbeddedInAssembly(
                Assembly.GetAssembly(typeof(PostgresDbUpRunner))!,
